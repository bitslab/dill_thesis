...
We compare the design and throughput of asynchronous delegation to that of  FFWD \cite{ffwd} and Gepard \cite{gepard}, as well as to the performance of other lock-free benchmarks. 
...
%One difference between delegation and locking is the time to acquire exclusive control of the memory block. Typically, threads acquiring the lock will continuously poll, or spin, on the lock, or yield the core to another thread until the lock is acquired. Either way the thread is unable to do useful work until the lock is acquired. Delegation servers have exclusive control over their assigned memory block and, therefore, do not have to wait to acquire a lock before they execute their critical section.  
