...
\section{Hardware}
A thread is a sequence of machine instructions running on a single central processing unit, or core. 
A core, or central processing unit, is the hardware responsible for executing instructions. 
A processor is a collection of cores, on the same physical unit, which may share resources.  
Non Uniform Memory Access (NUMA) describes a hardware organization scheme in a multi-processor system where a processor, or NUMA Node, has priority access to a portion of main memory. The result is reduced memory access latency for the portion of main memory local to the NUMA Node and increased memory latency for the remote portion of main memory. 

% Machine Organization Sockets - Cores - Threads
% NUMA
